...
import { getOwnerProducts, isUserPremium } from "@/lib/Business-server-action";
  ...
  return (
    <div className="mx-auto lg:w-3/5 py-10 px-6">
      {products.length === 0 ? (
        ...
        <div>
          <h1 className="text-3xl font-bold">Your Buisnesses</h1>
          <p>Manage your Buisness here</p>
...
<>Your Buisnesses are all listed below</>
          )}

          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mt-10">
            {products.map((product) => (
              <Link href={`/edit-Businessess/${product.id} `} key={product.id}>
                <div>
                  <div
                    className="
                            rounded-lg 
